...
--------------------tab 프로시저-----------------------
    ...
    begin
        ...
        for tab_list in 
            (
                select   *
                from     dba_tab_columns_syn --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다. 
                where    owner = f_user
        ...
        if i = 0 then -- 반복문을 거쳤는데 i가 0이면 
            raise not_result; --정의 exception 호출    
        end if ;
        
        -- 마지막 컬럼 뒤에 ,를 짜르기 위한 update문
        update  ddl_scripts 
        set     text = '    '||rtrim(r_column,',') 
        where   line = i 
        and     owner =f_user 
        and     object_name = f_tabname ;
...
--------------------source 프로시저--------------------
    ...
    begin 
        ...
        from dba_source_syn --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다.    
        where owner = f_user
        and name = f_objname;
            
            
        if sql%rowcount = 0 then -- 최근에 실행된 SQL문의 결과값이 아무것도 없으면
                raise not_result; --정의 exception 호출
        end if;
...
--------------------ind 프로시저-----------------------
    ...
    begin
        ...
        for ind_list in 
            (
                ...
                from    dba_ind_columns_syn  --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다.   
    ...
    begin
        ...
        (
             select *          
             from   dba_views_syn --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다.    
             where  owner = f_user
            ...
            set     text  = rtrim(text,';')||';' 
            where   line = i
            and     view_list.owner = f_user
            and     view_list.view_name=f_viewname;
...
procedure seq_proc
    (
        ...
        --index table 관련 변수
        type    v_temp_seq is table of dba_sequences_syn%rowtype -- 테이블의 타입을 그대로 복사하여 변수생성
    ...
    begin
        ...
        select  *
        into    v_temp(1)
        from    dba_sequences_syn  --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다.  
        ...
        v_text:= 
        'CREATE SEQUENCE '
        ||v_temp(1).sequence_name
        ||' INCREMENT BY '
        ||v_temp(1).increment_by
        ||' START WITH '
        ||v_temp(1).last_number
        ||' MINVALUE '
        ||v_temp(1).min_value
        ||' MAXVALUE '
        ||v_temp(1).max_value;
        ...
        if v_temp(1).cycle_flag = 'N' then
            v_text := v_text|| ' NOCYCLE ';
        else
            v_text := v_text|| ' CYCLE ';
        end if;           
            
        if v_temp(1).ORDER_FLAG = 'N' then
            v_text := v_text|| ' NORDER ';
        else
            v_text := v_text|| ' ORDER ';
        end if;

        if v_temp(1).cache_size != 0 then
            v_text :=  v_text||'CACHE '||v_temp(1).cache_size||' ';      
            i := i+1;
        end if;
...
procedure syn_proc
    ...
    begin 
        ...
        from        dba_synonyms_syn  --synonnym을 사용하여 다른 유저가 사용할때마다 권한을 주지않아도된다.  
        where       table_owner = f_user
