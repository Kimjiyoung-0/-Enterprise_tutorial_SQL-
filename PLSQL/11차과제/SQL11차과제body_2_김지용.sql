...
--------------------source 프로시저--------------------
    ...
    is
        ...
        v_column varchar2(100); -- CREAT OR REPLACE을 위한 변수
        v_type varchar(100);
        i binary_integer := 0; -- count를 위한 변수 
    ...
    begin 
        ...
        (
            ...
            i:= i+1;
            if i=1 then--첫번째 줄이면
                v_column := 'CREATE OR REPLACE'; -- 이문구를 변수에 넣는다
                if objlist.type = 'JAVA SOURCE' then--만약 타입이 자바소스면 
                    insert into ddl_scripts (owner, object_type, object_name,line,text)
                        values (f_user, objlist.type, f_objname, i,'CREATE OR REPLACE AND COMPILE JAVA SOURCE NAMED "'||f_objname||'" as ');--자바소스 형식에 맞게 insert한다.
                    i := i+1;
                    v_type := 'JAVA SOURCE';
                    v_column := '';
                    end if;
